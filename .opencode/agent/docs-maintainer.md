...
- Homelab-specific: homelab, taskfiles, opencodeignore
...
- Write markdown that fails linting (follow pre-flight checklist)
...
- **@pre-commit-reviewer**: Provides backup validation for markdown linting
- **@session-closer**: Documents work in CURRENT.md (also enforces markdown linting)
- **@pr-reviewer**: Validates docs completeness in PRs

**Note**: While @pre-commit-reviewer provides backup validation for markdown linting, you (docs-maintainer) are responsible for producing lint-clean markdown on first write by following the "Markdown Linting Requirements" section.

## Homelab-Specific Documentation Patterns

**Proxmox VMs:**

When documenting new VMs, always include:

- VM ID (from naming-conventions.md table)
- MAC address (for static DHCP assignments)
- Resource allocation (CPU, RAM, disk)
- Purpose and services running
- Network configuration (VLAN, static IP)

**Kubernetes Resources:**

When documenting K3s/Kubernetes deployments:

- Namespace location
- Helm chart version (if applicable)
- Custom values configuration
- Ingress/service exposure details
- Storage requirements (PVCs)

**Terraform Modules:**

When documenting infrastructure code:
...
When documenting automation:

- Target hosts/groups
- Required inventory variables
- SOPS-encrypted secrets referenced (without exposing values)
...
**Remember**: Always run `mkdocs build --strict` before completing your work. This is the single source of truth for documentation validation.
